...
import { GetGameStateUseCase, GameStateDTO } from '../../core/domain/use-cases/get-game-state.use-case.js';
...
export class GameApplicationService {
  ...
  calculateScores(gameId: string, impostorCorrectlyGuessed: boolean): Result<Map<string, number>, ResultError> {
    const game = this.gameManager.getGame(gameId);
    if (!game) {
      return { ok: false, error: new ResultError('GAME_NOT_FOUND', 'Game not found') };
    }
    const scores = this.calculateScoresUseCase.execute(game, impostorCorrectlyGuessed);
