...
vi.mock('@/features/game/components/copied-snackbar.vue', () => ({
  default: { name: 'CopiedSnackbar', template: '<div v-if="show">Copied!</div>', props: ['show'] },
}));

vi.mock('@/features/game/components/join-waiting-modal.vue', () => ({
  default: { name: 'JoinWaitingModal', template: '<div v-if="show">Waiting...</div>', props: ['show'] },
}));
...
describe('GameOrchestrator', () => {
  ...
  it('should rejoin if session exists for same game', async () => {
    const { getGameSession } = await import('@/shared/utils/session-storage.ts');
  ...
  it('should clear session if game ID differs', async () => {
    const { getGameSession, clearGameSession } = await import(
